      ...
      try {
        ...
        if (error instanceof HttpError) {
          ...
          if (error.statusCode === 400) {
            diagnosticInfo =
              'Bad request. Check your search parameters, especially cx (Search Engine ID).';
            ...
            if (error.message.includes('API key not valid')) {
              ...
              diagnosticInfo =
                'The API key has not been activated for the Custom Search API. Enable it in your Google Cloud Console.';
            } else if (error.message.includes('dailyLimit')) {
              diagnosticInfo =
                'You have exceeded your daily quota for the Google Custom Search API.';
            } else if (error.message.includes('userRateLimitExceeded')) {
              diagnosticInfo =
                'You are sending too many requests too quickly. Implement rate limiting in your application.';
            } else {
...
export const google = {
  ...
  search: async (_options: SearchOptions): Promise<SearchResult[]> => {
    throw new Error(
      'Google provider must be configured before use. Call google.configure() first.'
    );
  },
};
